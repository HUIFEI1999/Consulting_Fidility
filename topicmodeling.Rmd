...
We could use dplyr’s slice_max() to find the 10 terms that are most common within each topic and ceate a visualization.
```{r,warning=FALSE}
...
Now we use data after stemming and lemmatization to see if we have a improved results for topic identification. Because stemming would convert "plays" to "plai" which can be confusing for us to interpret the modeling results, we use lemmatized data to fit the model.
```{r}
...
lemma_top_terms %>%
  ...
  facet_wrap(~ topic, scales = "free") +
  scale_y_reordered()+
  labs(title = "lemmatization - LDA model results")
```
Based on the output, setting topic(K) to 6 cannot really help us distinguish one topic from the other one.
So we increase K to 10 given the data size of our dataset. 
...
lemma_lda <- LDA(lemma_dtm, k = 10, control = list(seed = 1234))
