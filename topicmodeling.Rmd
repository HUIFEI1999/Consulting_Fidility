...
IMDB_dtm
# IMDB_dtm has sparsity of 100%, but has more terms: 118916
```
...
Now we use data after stemming and lemmatization to see if we have a improved results for topic identification. Because stemming would convert "plays" to "plai" which may not be easy for us to interpret the modeling results, we use lemmatized data to fit the model.
```{r}
...
lemma_lda <- LDA(lemma_dtm, k = 6, control = list(seed = 1234))
...
```
Based on the output, setting topic(K) to 6 is cannot really help us distinguish one topic from the other one.
